...
// n자리의 오르막 수를 구하기 위해서는 n - 1자리의 숫자들의 오르막 수들의 마지막 숫자만
// 확인하면 된다는 사실을 이용하면 된다.
// 마지막 자리 숫자만 확인하면 오르막 수들은 앞의 숫자들은 오르막이라는 것을 보장하기 때문이다.
// 따라서 마지막 숫자별 갯수를 캐싱하면 된다.
//
// 이를 이용한 풀이에서 반복문에 최적화를 한번 더 할 수 있는데,
// n개 자리의 k로 끝나는 오르막 수는 n - 1개 자리의 k보다 작은 수들로 끝나는 오르막 수를 더하면 되므로
// k를 0부터 시작하여 8까지 더하면서 진행하면 반복문이 단순해진다. (17번줄)
#include <iostream>
#include <vector>
#define endl '\n'
using namespace std;
